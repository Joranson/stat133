...
# Your ggplot command:
p1 <- ggplot(wr1500m, aes(wr1500m$new_year, wr1500m$times_sec)) + xlim(1892, 2015) + geom_step() + geom_segment(aes(x = wr1500m[51,7], y = wr_1998, xend = 2015, yend = wr_1998))
p1
...
# Your ggplot command
p2 <- p1 + geom_vline(xintercept = wr1500m[wr1500m$year == 1944, 7], colour = "green") + annotate("text", label = wr1500m$athlete[wr1500m$year == 1944], x = 1944, y = 235, colour = "blue")
p2 <- p2 + geom_vline(xintercept = wr1500m[wr1500m$year == 1998, 7], colour = "green") + annotate("text", label = wr1500m$athlete[wr1500m$year == 1998], x = 1998, y = 215, colour = "blue")
p2
...
p3 = p2 + labs(x = "Record Setup Time (year)", y = "Times (sec)", title = "World Record of Men's 1500m from 1892 to 2014")
p3
...
s1 = ggplot(SO2012Ctry, aes(SO2012Ctry$pop,SO2012Ctry$GDP))+geom_point()
s1
...
# Your ggplot command
s2 <- ggplot(SO2012Ctry, aes(SO2012Ctry$pop, SO2012Ctry$GDP_per_person)) + scale_x_log10() + scale_y_log10() + geom_point(aes(size = SO2012Ctry$Total))
s2
...
# Your ggplot command:
s3 = s2 + labs(x = "Population (log)", y = "GDP per person (log)", title = "GDP per Person vs. Population and Olympic Medals")
for(i in 1:5){
  s3 <-s3 + annotate("text", label = SO2012Ctry$Country[top5[i]], SO2012Ctry$pop[top5[i]], SO2012Ctry$GDP_per_person[top5[i]])
}
s3
######################################
# PLOT 3.
...
# Your ggplot commands:
q1 = ggplot() + geom_polygon(aes(long, lat, group = group), color="black", fill = "light grey", data = map_data("world"))
q1
...
q2 = ggplot() + geom_polygon(aes(long, lat, group = group), fill = "grey40", data = map_data("world")) + 
  geom_point(aes(x = wonMedal$longitude, y = wonMedal$latitude, size = wonMedal$Total), colour = "gold")
q2
...
d1 = ggplot(athletes, aes(Sport, fill = Sex)) + geom_bar(position = "Dodge")
d1
...
# Your ggplot commands
d2 = d1+ theme(axis.text.x = element_text(angle = 90, hjust = 1)) + labs(title = "Participation of 2012 Olympics by Sports and Gender")
d2
