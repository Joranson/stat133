...
dice_sum <- function(k=2, B=100){
  sum_vec <- c()
  for (i in 1:B) {
    this_roll <- sample(x=1:6, size=k, replace=T)
...
even <- paste(rep("even",1000),seq(2,2000,2),sep="")
...
hotelCal.split=unlist(strsplit(hotelCal.split,"[[:space:]]"))
...
abbreviate <- function(vector, k){
  ...
  for (i in 1:length(vector)) {
    abbrv <- c(abbrv, substr(vector[i], start=0, stop=k))
  }
  return(abbrv)
}
